 ...
 * Plugin URI: 
 * Description: Show a YouTube playlist in an attractive, responsive format
 * Version: 1.0
...
function add_truman_youtube_stylesheet() 
{
    wp_enqueue_style( 'truman_youtube_style', plugins_url('/truman_youtube_display.css', __FILE__) );
    wp_enqueue_style( 'magnific-popup', plugins_url('/magnific-popup.css', __FILE__) ); 
    wp_enqueue_script( 'magnific-popup', plugins_url('/jquery.magnific-popup.min.js', __FILE__), array( 'jquery' ) );
}

add_action('init', 'add_truman_youtube_stylesheet');

/**
 * Adds Truman YouTube Display shortcode to wordpress
 * @author      Greg Marshall <gmarsh@truman.edu>
  * @return      void 
 */
function truman_youtube_shortcode($atts) 
{
...
add_shortcode( 'truman-youtube-display', 'truman_youtube_shortcode' );
